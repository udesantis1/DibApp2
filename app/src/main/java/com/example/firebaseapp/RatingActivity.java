...
public class RatingActivity extends AppCompatActivity {
    ...
    private Lesson lesson=new Lesson(course_id);
    private int media=0,somma=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       final String path = "Courses/" + course_id + "/Lessons/" + lesson_id + "/Rates";
               ...
               if(!lesson.getListVoti().containsKey(userMail)) {
                   btnSubmit.setOnClickListener(new View.OnClickListener() {
                       @Override
                       public void onClick(View v) {
                           int b = (int) ratingBar.getRating();
                           lesson.addUserAndVote(userMail, b);
                   ...
                   btnSubmit.setEnabled(false);
                   Toast.makeText(RatingActivity.this,"No vote",Toast.LENGTH_SHORT).show();
               }
    ...
    public int  media(HashMap <String ,Integer > lesson){
        ArrayList <Integer> voti= new ArrayList<>();
        voti.addAll((Collection<? extends Integer>) lesson);
        for(int i=0;i<voti.size();i++){
            somma=voti.get(i)+ somma;
